...
#    - On physionet-data project: 'BigQuery Data Viewer'
#      (or viewer on specific datasets)
BILLING_PROJECT_ID = (
    "level-strategy-383218"  # <<<--- REPLACE WITH YOUR BILLING PROJECT ID
)
...
try:
    ...
    if not BILLING_PROJECT_ID or BILLING_PROJECT_ID == "your-billing-project-id-here":
        raise ValueError(
            "BILLING_PROJECT_ID is not set. "
            "Please replace it with your actual Google Cloud project ID."
        )
    ...
    print(
        f"BigQuery client initialized successfully. "
        f"Billing project: {BILLING_PROJECT_ID}"
    )
    ...
    #     print(
    #         f"Warning: Could not directly confirm access to {test_dataset}. "
    #         f"Ensure permissions are set. Error: {e}"
    #     )
    ...
    print(
        f"3. Necessary IAM roles are granted on project "
        f"'{BILLING_PROJECT_ID}' and '{PHYSIONET_PROJECT}'."
    )
...
async def execute_bq_query(
    sql_query: str, query_params: Optional[list] = None
) -> pd.DataFrame | str:
    """
    Executes a BigQuery query using the global client and returns a DataFrame or
    error string.

    Args:
        sql_query: The SQL query string to execute.
        query_params: Optional list of bigquery.ScalarQueryParameter for
                     parameterized queries.

    Returns:
        A pandas DataFrame with the query results, or a string containing an
        error message.
    """
    if not bq_client:
        return (
    ...
    try:
        ...
        print(
            f"Executing BigQuery Query (Billing Project: {BILLING_PROJECT_ID}):"
            f"\n---\n{sql_query}\n---"
        )
        if query_params:
        ...
        query_job = bq_client.query(
            sql_query, job_config=job_config, project=BILLING_PROJECT_ID
        )
        ...
        error_message = f"Error executing BigQuery query: {e!s}"
        print(error_message)
        # Extract more specific BQ error details if available
        if hasattr(e, "errors") and e.errors:
            try:
                error_detail = e.errors[0].get(
                    "message", "No specific error message found."
                )
                reason_detail = e.errors[0].get("reason", "No specific reason found.")
                location_detail = e.errors[0].get(
                    "location", "No specific location found."
                )
                error_message += (
        ...
        if "db-dtypes" in str(e):
            error_message += (
                "\n\nHint: Try installing the required package: "
...
async def execute_arbitrary_mimic_query(sql_query: str) -> str:
    """
    Executes an arbitrary SQL SELECT query against the MIMIC datasets hosted on
    BigQuery.

    Args:
        sql_query: A valid BigQuery SQL SELECT query string. The query *must*
                  explicitly reference tables using the full path, e.g.,
                  `physionet-data.mimiciv_3_1_hosp.admissions`.

    Returns:
        A string containing the query results (formatted as a string table,
        max 50 rows) or an error message if the query fails or is disallowed.
    """
    ...
    for keyword in disallowed_keywords:
        if keyword in query_upper:
            return (
                f"Error: Query contains disallowed keyword ('{keyword.strip()}'). "
                f"Only SELECT queries are permitted."
            )

    # Enforce explicit project reference for clarity and safety
    required_project_prefix = f"`{PHYSIONET_PROJECT}`."  # Check backticks too
    required_project_prefix_no_ticks = f"{PHYSIONET_PROJECT}."
    sql_query_lower = sql_query.lower()
    if (
        required_project_prefix not in sql_query_lower
        and required_project_prefix_no_ticks not in sql_query_lower
    ):
        return (
            f"Error: Query must explicitly reference tables within the "
            f"'{PHYSIONET_PROJECT}' project "
...
@mcp.tool()
async def get_mimic_aggregation(metric: Literal["patient_count", "average_age"]) -> str:
    """
    ...
    if metric == "patient_count":
        ...
        SELECT
            AVG(CAST(
                ...
                AS NUMERIC
            )) as overall_average_admission_age
        FROM
        ...
        WHERE p.anchor_age IS NOT NULL
          ...
          -- Ensure admission is not before anchor year
        """
    else:
        return (
            f"Error: Unsupported metric '{metric}'. "
            f"Only 'patient_count' and 'average_age' are supported."
        )
    ...
    if isinstance(result, str):
        ...
        if result.empty or result.iloc[0, 0] is None:  # Check for empty/NULL result
            return (
                f"Aggregation query for '{metric}' executed successfully, "
                f"but returned no results (or zero count/null average)."
            )
        try:
...
@mcp.tool()
async def list_common_lab_items(
    search_term: Optional[str] = None, limit: int = 50
) -> str:
    """
    ...
    Args:
        search_term (Optional[str]): A term to search for within lab labels.
                                    Defaults to None (no filter).
        limit (int): Maximum number of lab labels to return. Defaults to 50.

    Returns:
        A JSON string containing a list of matching lab item labels,
        or an error message. Example output: '["Glucose", "Potassium", "Sodium"]'
    """
    if limit <= 0:
    ...
    if search_term:
        ...
        query_params.append(
            bigquery.ScalarQueryParameter(param_name, "STRING", f"%{search_term}%")
        )
...
async def find_admissions_with_criteria(
    ...
    lab_criteria: Optional[list[dict[str, Any]]] = None,
    ...
    Args:
        ...
        lab_criteria (Optional[list[dict[str, Any]]]): A list of lab criteria
            dictionaries. Each dictionary should have:
    ...
    Returns:
        A string containing a comma-separated list of matching hadm_ids
        (up to max_results), a count message, or an error message.
    """
    # --- Input Validation ---
    if max_results <= 0:
        return "Error: max_results must be positive."
    if min_age is not None and max_age is not None and min_age > max_age:
        return "Error: min_age > max_age."
    if gender not in [None, "M", "F"]:
        return "Error: gender must be 'M', 'F', or None."
        ...
        if parsed_lab_criteria:
            ...
            for item in parsed_lab_criteria:
                if not isinstance(item, dict) or "label" not in item:
                    raise ValueError(
                        "Each item in lab_criteria list must be an object "
                ...
                if (
                    ...
                    and not isinstance(item["value_min"], (int, float))
                ):
                    raise ValueError(
                        f"value_min for label '{item['label']}' must be a "
                        f"number or null."
                    )
                if (
                    "value_max" in item
                    and item["value_max"] is not None
                    and not isinstance(item["value_max"], (int, float))
                ):
                    raise ValueError(
                        f"value_max for label '{item['label']}' must be a "
                        f"number or null."
                    )
                # Check integer type for time window
                if (
                    "time_window_hours" in item
                    and item["time_window_hours"] is not None
                ):
                    if not isinstance(item["time_window_hours"], int):
                        raise ValueError(
                            f"time_window_hours for label '{item['label']}' "
    ...
    if (
        ...
        and max_age is None
        and gender is None
        and not parsed_lab_criteria
    ):
        return (
            "Error: Please provide at least one filter criterion "
            "(age, gender, or lab_criteria)."
        )
    ...
    lab_where_conditions = []  # Conditions from lab_criteria logic
    query_params = []
    param_counter = 0
    final_query_assembled = (
        False  # Flag to indicate if query was built by match_all logic
    )
    group_by_clause = ""
    having_clause = ""
    ...
    if min_age is not None or max_age is not None or gender is not None:
        ...
        demographic_joins.add(
            f"JOIN `{PHYSIONET_PROJECT}.{MIMIC_HOSP_DATASET}.patients` p "
            f"ON a.subject_id = p.subject_id"
        )
        age_calc = "(p.anchor_age + EXTRACT(YEAR FROM a.admittime) - p.anchor_year)"
        ...
        if min_age is not None:
            ...
            query_params.append(
                bigquery.ScalarQueryParameter(param_name, "INT64", min_age)
            )
        if max_age is not None:
            ...
            where_conditions.append(f"{age_calc} <= @{param_name}")
            query_params.append(
                bigquery.ScalarQueryParameter(param_name, "INT64", max_age)
            )
        if gender is not None:
            ...
            query_params.append(
                bigquery.ScalarQueryParameter(param_name, "STRING", gender)
            )

    # Define lab joins (if needed) and build lab filters
    # Stores conditions for each lab criterion: (cond1 AND cond2)
    lab_sub_clauses = []
    # Track unique labels required if match_all_labs=True
    required_labels = set()
    if parsed_lab_criteria:
        ...
        lab_join_clause = f"""
JOIN `{PHYSIONET_PROJECT}.{MIMIC_HOSP_DATASET}.labevents` le
ON a.hadm_id = le.hadm_id
JOIN `{PHYSIONET_PROJECT}.{MIMIC_HOSP_DATASET}.d_labitems` dli
ON le.itemid = dli.itemid"""
        ...
        for i, criterion in enumerate(parsed_lab_criteria):
            ...
            # Stores AND conditions for *this* criterion
            criterion_clause_parts = []
            ...
            query_params.append(
                bigquery.ScalarQueryParameter(param_name, "STRING", label)
            )
            ...
            if value_min is not None:
                ...
                query_params.append(
                    bigquery.ScalarQueryParameter(
                        param_name, "FLOAT64", float(value_min)
                    )
                )
            ...
            if value_max is not None:
                ...
                query_params.append(
                    bigquery.ScalarQueryParameter(
                        param_name, "FLOAT64", float(value_max)
                    )
                )
            ...
            if time_window is not None:
                ...
                criterion_clause_parts.append(
                    "le.charttime >= a.admittime"
                )  # Lab must be after admission start
                criterion_clause_parts.append(
                    f"TIMESTAMP_DIFF(le.charttime, a.admittime, HOUR) <= @{param_name}"
                )
                query_params.append(
                    bigquery.ScalarQueryParameter(param_name, "INT64", time_window)
                )
    ...
    if parsed_lab_criteria and match_all_labs and num_required_labs > 1:
        ...
        # Count unique labels required
        num_distinct_labels = len(required_labels)
        param_name = f"p{param_counter}"
        param_counter += 1
        having_clause = f" HAVING COUNT(DISTINCT dli.label) >= @{param_name}"
        query_params.append(
            bigquery.ScalarQueryParameter(param_name, "INT64", num_distinct_labels)
        )
        ...
        # Mark query as assembled for this specific case
        final_query_assembled = True

    # Case 2: Default - Match ANY lab, or only one lab specified, or no labs specified
    if not final_query_assembled:
        # Use DISTINCT if not grouping
        select_clause = "SELECT DISTINCT a.hadm_id"
        sql_query = f"{select_clause}\n{from_clause}\n"
    ...
    if isinstance(result, str):  # An error message string was returned
        ...
        if result.empty:
            return (
                "Query executed successfully, but found no admissions "
            ...
            return (
                f"Found {count} admissions matching criteria. "
                f"Returning hadm_ids: {ids_to_return}"
            )
    else:
        # Should not happen with current execute_bq_query logic
        return "Error: Unexpected return type from query execution."
...
if __name__ == "__main__":
    ...
    if not bq_client:
        ...
        print(
            "The server cannot execute BigQuery queries. "
            "Please check the error messages above."
        )
        print(
            "Ensure configuration, authentication, IAM roles, "
            "and API enablement are correct."
        )
        print("Exiting.")
        ...
        print(
            f"   - Requires full table paths (e.g., "
            f"`{PHYSIONET_PROJECT}.{MIMIC_HOSP_DATASET}.patients`)."
        )
        print(
            "2. get_mimic_aggregation(metric: Literal['patient_count', 'average_age'])"
        )
        print(
            "   - Simple overall aggregations "
            "(total patient count or average admission age)."
        )
        print("3. list_common_lab_items(search_term: Optional[str], limit: int = 50)")
        print(
            "   - lists available lab item labels, optionally filtered by search term."
        )
        # Updated description for find_admissions_with_criteria
        print(
            "4. find_admissions_with_criteria("
            "min_age: Optional[int], "
            "max_age: Optional[int], "
            "gender: Optional[Literal['M', 'F']], "
            "lab_criteria: Optional[list[dict[str, Any]]], "
            "match_all_labs: bool = True, "
            "max_results: int = 100)"
        )
        print(
            "   - Finds admission IDs (hadm_id) matching "
            "demographics and/or lab criteria."
        )
        print(
            "   - lab_criteria expects a list of dicts, "
            "e.g., [{'label': 'Glucose', 'value_max': 100}]"
        )

        # Start the FastMCP server
        print("\nStarting FastMCP server for MIMIC queries...")
        print(
            "Server ready to accept connections (e.g., via stdio). "

...
class OAuth2Config:
    ...
    def __init__(self):
        ...
        self.validate_exp = os.getenv("M3_OAUTH2_VALIDATE_EXP", "true").lower() == "true"
        self.validate_aud = os.getenv("M3_OAUTH2_VALIDATE_AUD", "true").lower() == "true"
        self.validate_iss = os.getenv("M3_OAUTH2_VALIDATE_ISS", "true").lower() == "true"
        
        # JWKS settings
        self.jwks_url = os.getenv("M3_OAUTH2_JWKS_URL", "")
        self.jwks_cache_ttl = int(os.getenv("M3_OAUTH2_JWKS_CACHE_TTL", "3600"))  # 1 hour
        
        # Rate limiting
        self.rate_limit_enabled = os.getenv("M3_OAUTH2_RATE_LIMIT_ENABLED", "true").lower() == "true"
        self.rate_limit_requests = int(os.getenv("M3_OAUTH2_RATE_LIMIT_REQUESTS", "100"))
        self.rate_limit_window = int(os.getenv("M3_OAUTH2_RATE_LIMIT_WINDOW", "3600"))  # 1 hour
    ...
    def _validate_config(self):
        ...
        if not self.jwks_url:
            ...
            self.jwks_url = urljoin(self.issuer_url.rstrip('/'), '/.well-known/jwks.json')
...
class OAuth2Validator:
    ...
    async def _get_jwks(self) -> Dict[str, Any]:
        ...
        if (self._jwks_cache and 
            current_time - self.config._jwks_cache_time < self.config.jwks_cache_ttl):
